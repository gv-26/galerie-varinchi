...
const targetStr = 'const currentImgs = pickerTargetCombo ? (comboImages[pickerTargetCombo] || []) : [];';
const targetIndex = content.indexOf(targetStr);
const endStr = '{/* -- Pricing Mode Toggle -- */}';
const endIndex = content.indexOf(endStr);

if (targetIndex !== -1 && endIndex !== -1) {
  const newContent = content.substring(0, targetIndex + targetStr.length) + '\n' +
                        const allUrls = folderImgs.map(img => img.url).filter(url => !currentImgs.includes(url));
          ...
          <div style={{
            ...
            <span style={{ fontSize: '13px', color: 'var(--color-text-muted)' }}>
              {pickerSelected.size > 0 ? \\ image\ selected\ : 'Click images to select'}
            </span>
            <div style={{ display: 'flex', gap: '10px' }}>
              ...
              >
                Add {pickerSelected.size > 0 ? \\ \ : ''}Image{pickerSelected.size !== 1 ? 's' : ''}
              </button>
            </div>
          </div>
        </div>
