...
export default function EditBlogPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const data = await res.json();
        const errorMessage = data.error || 'Failed to update blog';
        setError(errorMessage + (data.details ? `: ${data.details}` : ''));
        console.error('[EditBlog] Update failed:', data);
      }
    } catch (err: any) {
      setError('A network error occurred. Please check your connection or image sizes.');
      console.error('[EditBlog] Network error:', err);
    } finally {
      setLoading(false);
    }
  };
  ...
  if (initialLoading) return <div className="page-content" style={{ textAlign: 'center' }}><div className="spinner"></div></div>;
