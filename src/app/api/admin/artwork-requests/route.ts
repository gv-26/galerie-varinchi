...
export async function GET() {
  try {
    ...
    console.error('[admin/artwork-requests] GET error:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Keeping legacy route support — prefer /admin/artwork-requests/[id]
    ...
    console.error('[admin/artwork-requests] POST error:', error);
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 });
  }
}
